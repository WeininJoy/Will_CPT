...
# Other good-fit points (stored as OmegaM, OmegaK, Omegab_ratio, h)
...
other_good_fits[:, 2] *= other_good_fits[:, 0]   # Omegab = Omegab_ratio * OmegaM

# ── Planck 2018 MCMC chains ───────────────────────────────────────────────────
_planck_root = os.path.expanduser(
    '~/Documents/Research/PlanckData_2018/base_omegak/plikHM_TTTEEE_lowl_lowE/'
    'base_omegak_plikHM_TTTEEE_lowl_lowE')
_ns = read_chains(_planck_root)
_ns['omegab'] = _ns['omegabh2'] / _ns['H0']**2 * 100**2

# columns match the 4 plot axes: [OmegaM, OmegaK, Omegab, h]
planck_samps = np.column_stack([
    np.array(_ns['omegam']),
    np.array(_ns['omegak']),
    np.array(_ns['omegab']),
    np.array(_ns['H0']) / 100.0,
])
planck_chi2  = -2.0 * np.array(_ns.logL)        # chi2 = -2 * log-likelihood
planck_dchi2 = planck_chi2 - planck_chi2.min()   # delta-chi2 relative to global best-fit

_PC      = 'darkorange'
_PC_cmap = 'Oranges_r'

# parameter ranges: original CPT zoom (half the search span, centred on best-fit)
p_ranges = []
for i in range(4):
...
def _chi2_overlay_2d(ax, x, y, dchi2, cmap=_PC_cmap, color=_PC):
    """Contourf of Planck delta-chi2 for points already filtered to the panel region."""
    if len(x) < 8:
        return
    xi = np.linspace(x.min(), x.max(), 60)
    yi = np.linspace(y.min(), y.max(), 60)
    Xi, Yi = np.meshgrid(xi, yi)
    Zi = griddata((x, y), dchi2, (Xi, Yi), method='linear')
    Zi = np.ma.masked_invalid(Zi)
    if Zi.count() == 0:
        return
    levels = np.linspace(0, np.nanpercentile(dchi2, 95), 12)
    ax.contourf(Xi, Yi, Zi, levels=levels, cmap=cmap, alpha=0.65, zorder=4)
    ax.contour(Xi, Yi, Zi, levels=levels[::3], colors=[color],
               linewidths=0.6, alpha=0.8, zorder=5)


def _chi2_overlay_1d(ax, x, dchi2, color=_PC, bins=20):
    """Profile (minimum) delta-chi2 per parameter bin, plotted on a twin axis."""
    edges   = np.linspace(x.min(), x.max(), bins + 1)
    centers = 0.5 * (edges[:-1] + edges[1:])
    profile = np.array([dchi2[(x >= edges[k]) & (x < edges[k + 1])].min()
                        if ((x >= edges[k]) & (x < edges[k + 1])).any() else np.nan
                        for k in range(bins)])
    valid = np.isfinite(profile)
    if valid.sum() < 2:
        return
    ax2 = ax.twinx()
    ax2.plot(centers[valid], profile[valid], color=color, lw=1.5, zorder=3)
    ax2.fill_between(centers[valid], profile[valid],
                     np.nanmax(profile) * 1.05,
                     color=color, alpha=0.25, zorder=2)
    ax2.set_ylim(bottom=0)
    ax2.invert_yaxis()   # lower (better) chi2 points upward
    ax2.set_yticks([])
...
for row in range(n):
    for col in range(n):
        ...
        if row == col:
            ...
            for _gf in other_good_fits:
                ax.axvline(_gf[i], color='red', lw=0.8, ls=':', alpha=0.6)
            ...
            # ── Planck 2018 scatter ───────────────────────────────────────────
            _m = ((planck_samps[:, col] >= p_ranges[col][0]) &
                  (planck_samps[:, col] <= p_ranges[col][1]) &
                  (planck_samps[:, row] >= p_ranges[row][0]) &
                  (planck_samps[:, row] <= p_ranges[row][1]))
            if _m.sum() > 0:
                ax.scatter(planck_samps[_m, col], planck_samps[_m, row],
                           c=planck_dchi2[_m], cmap=_PC_cmap,
                           s=3, alpha=0.5, rasterized=True, zorder=3)
            ax.scatter(best_params[col], best_params[row],
                       marker='*', s=150, color='red', zorder=5)
            ax.scatter(other_good_fits[:, col], other_good_fits[:, row],
                       marker='*', s=60, color='red', alpha=0.6, zorder=5)
...
# ── Planck 2018 overlay: delta-chi2 from MCMC points filtered to each panel ───
for row in range(n):
    for col in range(n):
        ax = axes[row, col]
        if row == col:
            i   = col
            m1d = ((planck_samps[:, i] >= p_ranges[i][0]) &
                   (planck_samps[:, i] <= p_ranges[i][1]))
            if m1d.sum() > 1:
                _chi2_overlay_1d(ax, planck_samps[m1d, i], planck_dchi2[m1d])
        elif row > col:
            m2d = ((planck_samps[:, col] >= p_ranges[col][0]) &
                   (planck_samps[:, col] <= p_ranges[col][1]) &
                   (planck_samps[:, row] >= p_ranges[row][0]) &
                   (planck_samps[:, row] <= p_ranges[row][1]))
            if m2d.sum() > 0:
                _chi2_overlay_2d(ax, planck_samps[m2d, col],
                                 planck_samps[m2d, row], planck_dchi2[m2d])
            ax.set_xlim(p_ranges[col])
            ax.set_ylim(p_ranges[row])
...
# ── legend ───────────────────────────────────────────────────────────────────
_leg_handles = [
    mpatches.Patch(color='steelblue', label='Loss function'),
    mpatches.Patch(color=_PC,         label=r'Planck 2018 $\Delta\chi^2$'),
]
axes[0, n - 1].legend(handles=_leg_handles, fontsize=8, loc='upper left',
                      framealpha=0.8)
...
out = './figures/loss_param_space_with_Planck.pdf'
