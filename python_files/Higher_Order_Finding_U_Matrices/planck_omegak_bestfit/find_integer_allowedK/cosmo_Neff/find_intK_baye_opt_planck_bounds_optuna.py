...
m_ncdm = 0.06
epsilon = 1e-2
nu_spacing4_bestfit = [0.4447390978198118, np.float64(-0.03952117480452887),0.16167855679796067, 0.55768703176223, 3.046, 2.095762, 0.972835,0.053017]

##################
def calculate_z_rec(params):
    OmegaM, OmegaK, omega_b_ratio, h, Neff = params
    ...
    CMB_params = {
        ...
        'A_s': nu_spacing4_bestfit[5]*1e-9,
        'n_s': nu_spacing4_bestfit[6],
        'N_ncdm': N_ncdm,
        'm_ncdm': m_ncdm,
        'N_ur': Neff - N_ncdm,
        'tau_reio': nu_spacing4_bestfit[7],
        'lensing': 'no'
    }
...
# pbounds = {'OmegaM': (0.39, 0.5), 'OmegaK': (-0.045, -0.035), 'Omegab_ratio': (0.12, 0.19), 'h': (0.50, 0.61), 'Neff': (2.5, 3.5)}
pbounds = {'OmegaM': (0.38, 0.52), 'OmegaK': (-0.050, -0.030), 'Omegab_ratio': (0.12, 0.20), 'h': (0.50, 0.65), 'Neff': (2.5, 3.5)}
...
def objective(trial):
    ...
    Neff = trial.suggest_float('Neff', pbounds['Neff'][0], pbounds['Neff'][1])
    params = [OmegaM, OmegaK, Omegab_ratio, h, Neff]
    ...
    for attempt in range(10):
        try:
            ...
            study = optuna.create_study(
                study_name="planck_integer_k_optimization",
                storage=DB_URL,
                direction="minimize",
                sampler=sampler,
                load_if_exists=True 
