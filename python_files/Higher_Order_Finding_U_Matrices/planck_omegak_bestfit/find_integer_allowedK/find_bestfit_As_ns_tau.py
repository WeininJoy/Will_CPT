...
def objective(trial):
    # 2. Optuna natively handles your priors!
    As = trial.suggest_float('As', 1.5, 3.5)
    ns = trial.suggest_float('ns', 0.9, 1.1)
    tau = trial.suggest_float('tau', 0.03, 0.08)
    params = [As, ns, tau]
    
    # 3. Create a unique RAM-disk filename for THIS specific parallel worker
    worker_pid = os.getpid()
    unique_dat_name = f'/dev/shm/PPS_{worker_pid}.dat'
    
    # Initialize Planck Likelihood inside the worker
    lkl = PlanckLikelihood()
    
    # run_TT needs to be updated to accept lkl and unique_dat_name
    try:
        # Note: You can remove the old "check_priors" logic from run_TT 
        # because Optuna guarantees params are inside the box.
        ...
        print(f"Worker {worker_pid} failed: {e}")
        return 2e+30
    finally:
...
# def get_data_optuna():
#     # 1. Point Optuna to a shared database in the node's local memory (RAM)
#     db_path = "sqlite:////dev/shm/optuna_cmb.db"
    
#     # load_if_exists=True allows multiple independent Python scripts to join the same study
#     study = optuna.create_study(
#         study_name="cmb_optimization", 
#         storage=db_path, 
#         load_if_exists=True, 
#         direction="minimize"
#     )
    
#     # 2. RUN SEQUENTIALLY IN THIS SCRIPT (n_jobs=1). 
#     # Because we will launch 8 scripts from SLURM, 8 * 65 trials = 520 total trials.
#     print(f"Worker {os.getpid()} starting optimization...")
#     study.optimize(objective, n_trials=65)
    
#     # 3. Save the results safely. 
#     # Use the PID in the filename so the 8 workers don't overwrite each other simultaneously.
#     best = study.best_params
#     worker_file = f"{file_root}_worker_{os.getpid()}"
