...
# Define the bounds (must match the original optimizer)
pbounds = {
    'mt': (350, 500),
    'kt': (1.2, 1.8),
...
def plot_2d_scatter(x_obs, y_obs, z_obs, param1='mt', param2='kt'):
    ...
    ax.set_title('Bayesian Optimization: Observed Points', fontsize=14)

    cbar = plt.colorbar(scatter, ax=ax)
    cbar.set_label('-0.5 × loss_combined', fontsize=12)
    ...
    plt.tight_layout()
    plt.savefig('./bayes_opt_2d_scatter.pdf')
    print("Saved 2D scatter plot to: bayes_opt_2d_scatter.pdf")
    plt.show()
...
def plot_2d_gp_contour(X, Y, Z_pred, x_obs, y_obs, z_obs, param1='mt', param2='kt'):
    ...
    ax.set_xlabel(param1, fontsize=12)
    ax.set_ylabel(param2, fontsize=12)
    ax.set_title('Bayesian Optimization: GP Interpolation', fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig('./bayes_opt_2d_gp_contour.pdf')
    print("Saved 2D GP contour plot to: bayes_opt_2d_gp_contour.pdf")
    plt.show()
...
def plot_3d_surface(X, Y, Z_pred, x_obs, y_obs, z_obs, param1='mt', param2='kt'):
    ...
    ax.set_zlabel('-0.5 × loss_combined', fontsize=11)
    ax.set_title('Bayesian Optimization: GP Surface', fontsize=14)

    # Add colorbar
    cbar = fig.colorbar(surf, ax=ax, pad=0.1, shrink=0.7)
    cbar.set_label('-0.5 × loss_combined', fontsize=11)

    plt.tight_layout()
    plt.savefig('./bayes_opt_3d_gp_surface.pdf')
    print("Saved 3D GP surface plot to: bayes_opt_3d_gp_surface.pdf")
    plt.show()
...
def plot_uncertainty(X, Y, sigma, x_obs, y_obs, param1='mt', param2='kt'):
    ...
    plt.tight_layout()
    plt.savefig('./bayes_opt_gp_uncertainty.pdf')
    print("Saved GP uncertainty plot to: bayes_opt_gp_uncertainty.pdf")
    plt.show()
...
if __name__ == "__main__":
    ...
    print(f"  Target range: [{z_obs.min():.2f}, {z_obs.max():.2f}]")
    print(f"  Best target: {z_obs.max():.2f} at mt={x_obs[np.argmax(z_obs)]:.2f}, kt={y_obs[np.argmax(z_obs)]:.3f}")
    ...
    print("  1. bayes_opt_2d_scatter.pdf - Observed points only")
    print("  2. bayes_opt_2d_gp_contour.pdf - GP interpolation as contour")
    print("  3. bayes_opt_3d_gp_surface.pdf - GP interpolation as 3D surface")
    print("  4. bayes_opt_gp_uncertainty.pdf - GP uncertainty map")
