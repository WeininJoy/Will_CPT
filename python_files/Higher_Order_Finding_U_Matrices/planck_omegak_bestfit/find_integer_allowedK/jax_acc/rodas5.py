...
Modified to support batched Initial Conditions (y0).
"""
...
@functools.partial(jax.jit, static_argnames=("ode_fn", "lu_precision", "batch_size", "max_steps"))
def solve(ode_fn, y0, t_span, params, *, lu_precision: Literal["fp32", "fp64"] = "fp64", batch_size=None, rtol=1e-8, atol=1e-10, first_step=None, max_steps=100000):
    ...
    # Check if y0 is provided as a batch
    if y0_arr.ndim == 1:
    ...
    if n_padded > N:
        pad_rows = jnp.broadcast_to(params_arr[-1:], (n_padded - N,) + params_arr.shape[1:])
        params_padded = jnp.concatenate([params_arr, pad_rows], axis=0)

        pad_y0 = jnp.broadcast_to(y0_arr[-1:], (n_padded - N, n_vars))
        y0_padded = jnp.concatenate([y0_arr, pad_y0], axis=0)
    else:
    ...
    def _solve_batch(params_batch, y0_batch):
        ...
        def _step_batch(y, t, dt):
            ...
            du = f_eval(u, t_end)
            k6 = lu_solve(du + (_C61 * k1 + _C62 * k2 + _C63 * k3 + _C64 * k4 + _C65 * k5) * inv_dt)

            u = u + k6
            du = f_eval(u, t_end)
            k7 = lu_solve(du + (_C71 * k1 + _C72 * k2 + _C73 * k3 + _C74 * k4 + _C75 * k5 + _C76 * k6) * inv_dt)

            u = u + k7
            du = f_eval(u, t_end)
            k8 = lu_solve(du + (_C81 * k1 + _C82 * k2 + _C83 * k3 + _C84 * k4 + _C85 * k5 + _C86 * k6 + _C87 * k7) * inv_dt)
        ...
        def body_fn(state):
            ...
            dt_use = jnp.where(active, jnp.maximum(jnp.minimum(dt, next_target - t), 1e-30), 1e-30)
            ...
            safe_err = jnp.where(jnp.isnan(err_norm) | (err_norm > 1e18), 1e18, jnp.where(err_norm == 0.0, 1e-18, err_norm))
