# -*- coding: utf-8 -*-
"""
Refactored to use JAX & Diffrax for ultra-fast vectorized ODE solving,
now spanning 100% of CPU cores natively via ThreadPoolExecutor.
"""
import os
# ---> ADD THESE 4 LINES AT THE VERY TOP <---
# This completely disables JAX/NumPy internal multi-threading
...
def compute_U_matrices_Jax(params, z_rec, folder_path, n_processes=None, kvalues=None):
    ...
    # JAX PARALLEL BATCH COMPILER FOR PERTURBATION MATRICES 
    ...
    @jax.jit
    def process_single_k_jax(k):
        # Using Dopri8 explicit solver (fast but requires k > 1.0 to avoid stiffness)
        solver = diffrax.Dopri8()
        step_ctrl = diffrax.PIDController(rtol=rtol, atol=atol)

        def solve_single_ic(y0):
            sol1 = diffrax.diffeqsolve(
                diffrax.ODETerm(dX2_jax), solver, t0=_t_end, t1=_t_sw, dt0=None,
                y0=y0, args=k, stepsize_controller=step_ctrl,
                saveat=diffrax.SaveAt(t1=True), max_steps=int(5e5)
            )
            y1 = sol1.ys[0]
            y1 = y1.at[0].set(jnp.log(y1[0]))

            sol2 = diffrax.diffeqsolve(
                diffrax.ODETerm(dX3_jax), solver, t0=_t_sw, t1=_t_rec, dt0=None,
                y0=y1, args=k, stepsize_controller=step_ctrl,
                saveat=diffrax.SaveAt(t1=True), max_steps=int(5e5)
            )
            return sol2.ys[0][1:]

        # Create all 9 initial conditions
        abc_x0 = jnp.eye(NV)[:6]
        ...
        s_col = jnp.full((9, 1), _s_init)
        all_inits = jnp.concatenate([s_col, all_x0], axis=1)  

        # Map tightly across the 9 conditions using the C++ equivalent of a for-loop
        y_final = jax.lax.map(solve_single_ic, all_inits)

        ABC_matrix = y_final[0:6].T
        DEF_matrix = y_final[6:8].T
        GHI_vector = y_final[8]
    ...
    print(f"JIT Compiling ODE system (Warm-up)...")
    # Warm-up the JIT compiler exactly once before distributing to threads
    _ = process_single_k_jax(jnp.array(kvalues[0]))
    
    print(f"Executing JAX on {n_processes} CPU Threads simultaneously...")

    # -------------------------------------------------------------------------
    # Using ThreadPoolExecutor to max out 100% of the CPU cores natively in JAX!
    # -------------------------------------------------------------------------
    with ThreadPoolExecutor(max_workers=n_processes) as executor:
        results = list(executor.map(process_single_k_jax, jnp.array(kvalues), chunksize=1))

    ABCmatrices = np.array([r[0] for r in results])
    DEFmatrices = np.array([r[1] for r in results])
    GHIvectors  = np.array([r[2] for r in results])
    X1matrices  = np.array([r[3] for r in results])
    X2matrices  = np.array([r[4] for r in results])
    ...
    return {
        'kvalues': kvalues,
        'ABCmatrices': ABCmatrices,
        'DEFmatrices': DEFmatrices,
        'GHIvectors': GHIvectors,
        'X1matrices': X1matrices,
        'X2matrices': X2matrices
    }