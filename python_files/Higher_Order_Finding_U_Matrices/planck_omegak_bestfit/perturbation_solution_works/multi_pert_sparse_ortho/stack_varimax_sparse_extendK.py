# -*- coding: utf-8 -*-
"""
Stacked Varimax Rotation for Joint Sparse Orthonormal Solutions (WITH FULL EXTENSION)

This script applies Stacked Varimax rotation INCLUDING extended K modes in eigenvalue analysis.
This is for comparison with the divide-and-conquer approach.

Warning: High-k perfect eigenvalues (1.0) may dominate the results.

Based on stack_varimax_sparse.py but using full extension before eigenvalue analysis.
"""
import numpy as np
...
import sys
sys.path.append('..')
# from multi_perturbation_analysis import multi_perturbation_analysis_with_full_extension
from multi_perturbation_analysis import multi_perturbation_extension_reverse
...
def plot_sparse_eigenfunctions(results, sparse_results, N_plot=5,
    ...
    This shows solutions sparse in BOTH bases overlapped to verify they match.
    ...
    coefficients_sparse_2 = sparse_results['coefficients_sparse_2']
    ...
    fig.suptitle("Stacked Varimax Sparse Eigenfunctions (Both Bases Overlapped)", fontsize=16)
    ...
    for i in range(N_plot):
        ...
        for j, pert_type in enumerate(perturbation_types):
            ...
            if pert_type not in coefficients_sparse_1 or pert_type not in coefficients_sparse_2:
                ax.text(0.5, 0.5, f"No data\nfor {pert_type}",
                       ha='center', va='center', transform=ax.transAxes)
                ax.set_title(f"{pert_type} (eigenval {plot_idx+1})")
                continue

            # Reconstruct solution from sparse coefficients - Basis 1
            solution_sparse_1 = np.zeros_like(eta_grid)
            coeffs_1 = coefficients_sparse_1[pert_type][plot_idx, :]
            for k in range(len(coeffs_1)):
                solution_sparse_1 += coeffs_1[k] * basis_1[pert_type][:, k]

            # Reconstruct solution from sparse coefficients - Basis 2
            solution_sparse_2 = np.zeros_like(eta_grid)
            coeffs_2 = coefficients_sparse_2[pert_type][plot_idx, :]
            for k in range(len(coeffs_2)):
                solution_sparse_2 += coeffs_2[k] * basis_2[pert_type][:, k]

            # Sign alignment
            if np.dot(solution_sparse_1, solution_sparse_2) < 0:
                solution_sparse_2 *= -1

            # Plot both
            ax.plot(eta_grid, solution_sparse_1, 'r-', linewidth=2.5,
                   label='Basis 1 (Closed)', alpha=0.8)
            ax.plot(eta_grid, solution_sparse_2, 'g--', linewidth=2.0,
...
def plot_sparse_vs_original_eigenfunctions(results, sparse_results, N_plot=3,
    ...
    """
    Compare original and sparse eigenfunctions side by side with both bases overlapped.
    ...
    coefficients_original_2 = sparse_results['coefficients_original_2']
    coefficients_sparse_1 = sparse_results['coefficients_sparse_1']
    coefficients_sparse_2 = sparse_results['coefficients_sparse_2']
    basis_1 = results['basis_1']
    basis_2 = results['basis_2']
    ...
    fig.suptitle(f"Original vs Stacked Varimax Sparse Eigenfunctions ({reference_pert}, Both Bases)", fontsize=14)
    ...
    for i in range(N_plot):
        ...
        # Original eigenfunction - both bases
        ax_orig = axes[i, 0]

        # Basis 1
        solution_orig_1 = np.zeros_like(eta_grid)
        coeffs_orig_1 = coefficients_original_1[reference_pert][plot_idx, :]
        for k in range(len(coeffs_orig_1)):
            solution_orig_1 += coeffs_orig_1[k] * basis_1[reference_pert][:, k]

        # Basis 2
        solution_orig_2 = np.zeros_like(eta_grid)
        coeffs_orig_2 = coefficients_original_2[reference_pert][plot_idx, :]
        for k in range(len(coeffs_orig_2)):
            solution_orig_2 += coeffs_orig_2[k] * basis_2[reference_pert][:, k]

        # Sign alignment
        if np.dot(solution_orig_1, solution_orig_2) < 0:
            solution_orig_2 *= -1

        ax_orig.plot(eta_grid, solution_orig_1, 'r-', linewidth=2.0,
                    label='Basis 1', alpha=0.8)
        ax_orig.plot(eta_grid, solution_orig_2, 'g--', linewidth=1.5,
                    label='Basis 2', alpha=0.8)
        ...
        # Sparse eigenfunction - both bases
        ax_sparse = axes[i, 1]

        # Basis 1
        solution_sparse_1 = np.zeros_like(eta_grid)
        coeffs_sparse_1 = coefficients_sparse_1[reference_pert][plot_idx, :]
        for k in range(len(coeffs_sparse_1)):
            solution_sparse_1 += coeffs_sparse_1[k] * basis_1[reference_pert][:, k]

        # Basis 2
        solution_sparse_2 = np.zeros_like(eta_grid)
        coeffs_sparse_2 = coefficients_sparse_2[reference_pert][plot_idx, :]
        for k in range(len(coeffs_sparse_2)):
            solution_sparse_2 += coeffs_sparse_2[k] * basis_2[reference_pert][:, k]
        ...
        dominant_k = max_indices[plot_idx]
        ax_sparse.plot(eta_grid, solution_sparse_1, 'r-', linewidth=2.0,
                      label='Basis 1', alpha=0.8)
        ax_sparse.plot(eta_grid, solution_sparse_2, 'g--', linewidth=1.5,
                      label='Basis 2', alpha=0.8)
        ax_sparse.set_title(f"Stacked Varimax (dominant k={dominant_k+1})", fontsize=10)
        ax_sparse.grid(True, alpha=0.3)
        if i == 0:
            ax_sparse.legend(fontsize=8)
        if i == N_plot - 1:
...
def stacked_varimax_sparse_analysis(results, eigenvalue_threshold=0.99,
    ...
    plot_joint_sparse_coefficients(
        ...
        eigenvals_valid,
        N_plot=N_plot,
        output_filename="./figures/stack_varimax_coefficients_fullExtension.pdf"
    )
    ...
    plot_sparsity_comparison(
        ...
        eigenvals_valid,
        N_plot=min(N_plot, 5),
        output_filename="./figures/stack_varimax_sparsity_comparison_fullExtension.pdf"
    )
    ...
    plot_joint_sparsity_heatmap(
        ...
        coefficients_sparse_2_dict[reference_pert],
        eigenvals_valid,
        output_filename="./figures/stack_varimax_heatmap_fullExtension.pdf"
    )
    ...
    plot_dominant_mode_correlation(
        ...
        coefficients_sparse_2_dict[reference_pert],
        eigenvals_valid,
        output_filename="./figures/stack_varimax_correlation_fullExtension.pdf"
    )
    ...
    # Save to pickle
    with open("stack_varimax_sparse_results_fullExtension.pickle", 'wb') as f:
        pickle.dump(sparse_results, f)
    print("\nSaved results to stack_varimax_sparse_results_fullExtension.pickle")
...
if __name__ == "__main__":
    ...
    results_file = "../multi_perturbation_results_fullExtension.pickle"

    if os.path.exists(results_file):
        print("Loading existing multi_perturbation_analysis_with_full_extension results...")
        ...
        if 'basis_2' not in results or 'eigenvecs_2' not in results:
            ...
            print("Please re-run multi_perturbation_analysis_with_full_extension with both bases enabled.")
            sys.exit(1)
    else:
        print("No existing results found, running multi_perturbation_analysis_with_full_extension...")
        folder_path = f'../data/'
        ...
        # results = multi_perturbation_analysis_with_full_extension(N=N_allowedK, N_t=1000, folder_path=folder_path)
        results = multi_perturbation_extension_reverse(N=N_allowedK, N_t=1000, folder_path=folder_path)
    ...
    if sparse_results is not None:
        print("\n" + "="*80)
        print("STACKED VARIMAX ANALYSIS COMPLETE (FULL EXTENSION)")
        print("="*80)
        ...
        print(f"\nOutput files:")
        print("  - stack_varimax_sparse_results_fullExtension.pickle")
        print("  - stack_varimax_coefficients_fullExtension.pdf")
        print("  - stack_varimax_sparsity_comparison_fullExtension.pdf")
        print("  - stack_varimax_heatmap_fullExtension.pdf")
        print("  - stack_varimax_correlation_fullExtension.pdf")
        ...
        plot_sparse_eigenfunctions(results, sparse_results, N_plot=5,
                                    output_filename="./figures/stack_varimax_sparse_eigenfunctions_fullExtension.pdf")
        plot_sparse_vs_original_eigenfunctions(results, sparse_results, N_plot=3,
                                                output_filename="./figures/stack_varimax_sparse_vs_original_fullExtension.pdf")

        print("\nAdditional output files:")
        print("  - stack_varimax_sparse_eigenfunctions_fullExtension.pdf")
        print("  - stack_varimax_sparse_vs_original_fullExtension.pdf")
    else:
