...
- basis_1 is constructed from a range of wavenumbers 'k' in a closed universe model.
- basis_2 is constructed from the specific 'allowed' wavenumbers for a palindromic flat universe.

Both bases represent the full evolution from the Big Bang to the Future Conformal Boundary.
"""
import numpy as np
...
# =============================================================================
# SECTION 1: FULL SOLUTION GENERATION
# This section contains the full logic to generate the complete phi(eta)
# solution from the Big Bang to the FCB for a given k.
# =============================================================================

# --- Cosmological Parameters and Setup ---
folder_path = './data/'
h = 0.5409; H0 = 100*h; Omega_gamma_h2 = 2.47e-5; Neff = 3.046
...
# --- ODE Functions (Copied from previous script) ---
...
# --- Main Full Solution Generation Function ---
def generate_full_solution(k, is_allowed_k=False):
    """
    Generates the COMPLETE phi(eta) solution from Big Bang to FCB.
    If is_allowed_k is True, it assumes vr_inf is zero. Otherwise, it needs to be calculated.
    Returns a time array and the corresponding phi solution array.
    """
    # Get transfer matrices for this k
    ...
    try:
        ...
        print(f"Warning: Could not solve for x_inf for k={k}. Solution may be inaccurate.")
        x_inf = np.zeros(4) # Fallback

    # If this is an allowed_k, we know vr_inf should be zero. Let's enforce it.
    if is_allowed_k:
        x_inf[2] = 0.0

    # Calculate full solution vector Y_FCB AT the FCB
    dr_inf, dm_inf, vr_inf, vmdot_inf = x_inf
    phi_inf = (X1 @ x_inf)[0]; psi_inf = phi_inf
    vm_inf = -(vmdot_inf + psi_inf) / Hinf
    Y_FCB = np.zeros(num_variables_boltzmann)
    Y_FCB[0] = 0.0; Y_FCB[1:7] = [phi_inf, psi_inf, dr_inf, dm_inf, vr_inf, vm_inf]

    # Calculate initial conditions at eta' (endtime)
    ...
    mask = t_backward >= swaptime; Y_backward[0, mask] = np.exp(Y_backward_sigma[0, mask])
    ...
    phi0 = 1 + phi1*t0_integration + phi2*t0_integration**2
    Y0_perfect = [sigma0, phi0, 0, 0, 0, 0] # simplified for brevity, full calculation needed
    ...
    # Stitch the solution from BB to FCB
    t_left = np.concatenate((sol_perfect.t, t_backward[::-1], [fcb_time]))
    
    # Extract phi from perfect fluid and backward integration parts
    phi_perfect = sol_perfect.y[1, :]
    phi_backward = Y_backward[1, ::-1] # a.k.a phi
    
    phi_left = np.concatenate((phi_perfect, phi_backward, [phi_inf]))
    
    # Sort by time to ensure monotonicity for interpolation
    sort_indices = np.argsort(t_left)
    t_left_sorted = t_left[sort_indices]
    phi_left_sorted = phi_left[sort_indices]
    
    return t_left_sorted, phi_left_sorted
...
def compute_coefficients(eigenvalues, eigenvectors, transformation_matrix):
    coefficients = np.zeros((len(eigenvalues), transformation_matrix.shape[1]))
...
if __name__=="__main__":
    ...
    kt = -OmegaK / (np.sqrt(OmegaLambda * OmegaR) / 3); print(f"Calculated kt = {kt:.4f}")
    try:
    ...
    for i in range(1, N + 1):
        ...
        t_sol, phi_sol = generate_full_solution(k_effective, is_allowed_k=False)
    ...
    for k_val in allowed_K_basis:
        ...
        t_sol, phi_sol = generate_full_solution(k_val, is_allowed_k=True)
        phi_interpolated = interp1d(t_sol, phi_sol, bounds_error=False, fill_value=0.0)
    ...
    if N_plot > 0:
        ...
        for i in range(N_plot):
            ax = axs[i]
            # Reconstruct the eigenfunction from basis 1
            solution_1 = np.zeros_like(eta_grid, dtype=float)
            for j in range(N):
                solution_1 += coefficients_1[i, j] * basis_1[j]

            # Reconstruct the eigenfunction from basis 2
            solution_2 = np.zeros_like(eta_grid, dtype=float)
            for j in range(N):
                solution_2 += coefficients_2[i, j] * basis_2[j]

            # Normalize by making the initial point positive for consistent comparison
            ...
            ax.set_ylabel(f"Eigenfunc. {i+1}")
            ax.legend()

        axs[-1].set_xlabel(r"Conformal Time $\eta$")
        plt.savefig(f"phi_N{N:d}_Nt{N_t:d}.pdf")
    else:
