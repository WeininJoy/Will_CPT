...
try:
    ...
    allowedK = [i*np.sqrt(K) for i in range(1, 30)] # discrete integer k, with 4*integer
...
for i, sol in enumerate(solutions):
    ...
    for j, l in enumerate(ells):
        integral_rec, err_rec = quad(rec_integrand, rec_start_eta, rec_end_eta,
                                     args=(l, k_n, interp, recConformalTime), limit=100)
...
print(f"Sample k values: {allowedK[:5]}")
print(f"Sample P_R values: {P_R_values[:5]}")
print(f"Max Theta_l_k value: {np.max(np.abs(Theta_l_k)):.6e}")
print(f"Sample Theta_l_k[0,:5]: {Theta_l_k[0,:5]}")
...
plt.legend()
plt.tight_layout()
plt.savefig(folder_path + 'CMB_Power_Spectrum_Corrected.pdf')
plt.show()