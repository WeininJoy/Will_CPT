...
for i in range(num_modes_to_plot):
    ...
    sol2 = solve_ivp(ds_dt, [t0_integration,endtime], [s_bang_init], method='LSODA', events=reach_FCB, atol=atol, rtol=rtol)
    s_init = sol2.y[0,-1]

    Y_prime = np.zeros(num_variables_boltzmann+1) # +1 for s
    Y_prime[0] = s_init
    Y_prime[1:7] = x_prime
    ...
    sol_part1 = solve_ivp(dX2_dt, [endtime, swaptime], Y_prime, method='LSODA', atol=atol, rtol=rtol)
    
    Y_swap = sol_part1.y[:, -1]

    # Use a-evolution for second part
    Y_swap[0] = 1/Y_swap[0]
    ...
    Y_backward_a= np.concatenate((sol_part1.y, sol_part2.y), axis=1)
    Y_backward = Y_backward_a.copy()
    ...
    sol_perfect = solve_ivp(dX1_dt, [t0_integration, recConformalTime], Y0_perfect,
                            dense_output=True, method='LSODA', atol=atol, rtol=rtol)

    # --- f) Stitch and Reflect ---
    t_left = np.concatenate((sol_perfect.t, t_backward[::-1]))

    Y_perfect_full = np.zeros((num_variables_boltzmann+1, len(sol_perfect.t)))
    ...
    symm = np.ones(num_variables_boltzmann+1)
    symm[[5, 6]] = -1
...
plt.savefig(folder_path + 'perturbation_solutions_evol_a.pdf')
plt.show()