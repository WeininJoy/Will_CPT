...
def process_single_k(k, s_init, endtime, swaptime, recConformalTime, deltaeta,
                     Hinf, OmegaM, OmegaLambda, OmegaR, OmegaK, s0,
    ...
    deltaeta : float
        Time step parameter
    Hinf, OmegaM, OmegaLambda, OmegaR, OmegaK, s0 : float
    ...
    def dX2_dt_local(t, X):
        ...
        vmdot = (sdot/s)*vm - psi
        derivatives = [sdot, phidot, drdot, dmdot, vrdot, vmdot, fr2dot]

        for j in range(7, num_variables):
            l = j - 4
    ...
    def dX3_dt_local(t, X):
        sigma, phi, dr, dm, vr, vm, fr2 = X[0:7]
        ...
        # poisson_residual = phi + 3*H0**2 / (2*(k**2 + 3*OmegaK*H0**2)) * ( (3*sigmadot*vm + dm)*np.exp(sigma)*OmegaM + (4*sigmadot*vr + dr)*np.exp(2*sigma)*OmegaR)
        psi = phi - 3*H0**2 * OmegaR/k**2 * np.exp(2*sigma) * fr2

        phi = -3*H0**2 / (2*(k**2 + 3*OmegaK*H0**2)) * ( (3*sigmadot*np.exp(sigma)*vm + dm)*np.exp(sigma)*OmegaM + (4*sigmadot*np.exp(sigma)*vr + dr)*np.exp(sigma)**2*OmegaR)
        psi = phi - (1/k**2*np.exp(-2*sigma)) * dr * fr2
        ...
        derivatives = [sigmadot, phidot, drdot, dmdot, vrdot, vmdot, fr2dot]
    ...
    ABC_matrix = np.zeros(shape=(num_variables, 5))
    ...
    for n in range(5):
        ...
        solperf = solve_ivp(dX2_dt_local, [endtime, swaptime], inits, method='Radau', atol=atol, rtol=rtol,max_step=1.e-2)

        inits2 = solperf.y[:, -1]
        inits2[0] = np.log(inits2[0])

        solperf2 = solve_ivp(dX3_dt_local, [swaptime, recConformalTime], inits2, method='Radau', atol=atol, rtol=rtol,max_step=1.e-2)
    ...
    for j in range(0, 2):
        ...
        sol3 = solve_ivp(dX2_dt_local, [endtime, swaptime], inits, method='Radau', atol=atol, rtol=rtol,max_step=1.e-2)

        inits2 = sol3.y[:, -1]
        inits2[0] = np.log(inits2[0])

        sol4 = solve_ivp(dX3_dt_local, [swaptime, recConformalTime], inits2, method='Radau', atol=atol, rtol=rtol,max_step=1.e-2)
    ...
    # sol5 = solve_ivp(dX2_dt_local, [endtime, swaptime], inits, method='BDF', atol=atol, rtol=rtol)

    # inits2 = sol5.y[:, -1]
    # inits2[0] = np.log(inits2[0])

    # sol6 = solve_ivp(dX3_dt_local, [swaptime, recConformalTime], inits2, method='BDF', atol=atol, rtol=rtol)
    ...
    return ABC_matrix, DEF_matrix, X1, X2
...
# allowedK_integer_path = f'./nu_spacing{nu_spacing}_intK/data_all_k/allowedK_integer.npy'
...
# Note: s0 is already calculated at line 85 as the initial scale factor
# s_init is the value at endtime, which is different from s0
# For X1, X2 calculations, we need to use s0 (not s_init)
...
worker_func = partial(
    ...
    OmegaR=OmegaR,
    OmegaK=OmegaK,
    s0=s0,  # Use the s0 calculated at line 85, NOT s_init
...
# Unpack results
for ABC_matrix, DEF_matrix, X1, X2 in results:
