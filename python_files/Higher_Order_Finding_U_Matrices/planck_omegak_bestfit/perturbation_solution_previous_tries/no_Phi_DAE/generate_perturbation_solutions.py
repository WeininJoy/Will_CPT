...
# Data folder
nu_spacing = 4
folder = './data/'# f'./data/nu_spacing{nu_spacing}_s-a/'
...
# It is an Identity matrix, but with a 0 on the diagonal for the algebraic variable.
def get_mass_matrix(num_variables):
    M = np.eye(num_variables)
    # Assuming index 1 is 'phi', which is the algebraic constraint
    M[1, 1] = 0 
    # Use a sparse matrix if num_variables is very large for performance
    return M
...
for i in range(num_modes_to_plot):
    ...
    # set phi_prime and psi_prime to derived values to ensure constraint satisfaction
    Y_prime[1] = phi_derived
    Y_prime[2] = phi_derived
    
    # --- d) Integrate backwards from endtime to recConformalTime ---
    M = get_mass_matrix(num_variables+1) # +1 for s

    # Solve
    sol_part1 = solve_ivp(dX2_dt_local, [endtime, swaptime], Y_prime, 
        method='Radau',      # 'Radau' or 'BDF' are required for Mass Matrix problems
        mass_matrix=M,       # Pass the mass matrix here
        rtol=1e-6, atol=1e-9
    )
    ...
    sol_part2 = solve_ivp(dX3_dt_local, [swaptime, recConformalTime], Y_swap, 
                              method='Radau', mass_matrix=M, rtol=1e-6, atol=1e-9)
