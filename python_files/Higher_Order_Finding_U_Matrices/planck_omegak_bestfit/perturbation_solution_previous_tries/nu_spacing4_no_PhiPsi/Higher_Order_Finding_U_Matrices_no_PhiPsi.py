...
def process_single_k(k, s_init, endtime, swaptime, recConformalTime, deltaeta,
    ...
    def dX2_dt_local(t, X):
        ...
        phi = -3*H0**2 / (2*(k**2 + 3*OmegaK*H0**2)) * ( (3*sdot/s*vm + dm)*s*OmegaM + (4*sdot/s*vr + dr)*s**2*OmegaR)
        ...
        for j in range(6, num_variables):
            l = j - 3
    ...
    ABC_matrix = np.zeros(shape=(num_variables, 4))
    ...
    for n in range(4):
        ...
        solperf = solve_ivp(dX2_dt_local, [endtime, swaptime], inits, method='BDF', atol=atol, rtol=rtol)

        inits2 = solperf.y[:, -1]
        inits2[0] = np.log(inits2[0])

        solperf2 = solve_ivp(dX3_dt_local, [swaptime, recConformalTime], inits2, method='BDF', atol=atol, rtol=rtol)
    ...
    for j in range(0, 2):
        ...
        sol3 = solve_ivp(dX2_dt_local, [endtime, swaptime], inits, method='BDF', atol=atol, rtol=rtol)

        inits2 = sol3.y[:, -1]
        inits2[0] = np.log(inits2[0])

        sol4 = solve_ivp(dX3_dt_local, [swaptime, recConformalTime], inits2, method='BDF', atol=atol, rtol=rtol)
