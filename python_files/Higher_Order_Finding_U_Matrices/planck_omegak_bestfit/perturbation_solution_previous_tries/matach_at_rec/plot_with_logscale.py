# -*- coding: utf-8 -*-
"""
Plot perturbation evolution with logarithmic time scale
to show there's no jump at the Big Bang
"""
...
#-------------------------------------------------------------------------------
# Solve for k=0.5 and k=1.0 with root-finding
#-------------------------------------------------------------------------------
...
for plot_idx, k in enumerate(k_test_values):
    ...
    print(f"  phi0 = {phi0_optimal:.6f}")

    # Solve with optimal phi0
    sol3_plot = solve_for_phi0(phi0_optimal)

    # Create logarithmic time array for plotting
    t_plot = np.logspace(np.log10(t0), np.log10(recConformalTime), 2000)
    ...
    ax_phi.scatter([t0], [phi0_optimal], color='red', s=100, zorder=5, label=f'IC at t₀={t0:.0e}')
    ax_phi.axvline(recConformalTime, color='gray', linestyle='--', alpha=0.5, linewidth=1.5, label='Recombination')
    ax_phi.set_xlabel('Conformal time (log scale)', fontsize=12)
    ...
    ax_d.scatter([t0], [dr0], color='green', s=80, zorder=5, marker='o')
    ax_d.scatter([t0], [dm0], color='magenta', s=80, zorder=5, marker='o')
    ax_d.axvline(recConformalTime, color='gray', linestyle='--', alpha=0.5, linewidth=1.5)
    ax_d.set_xlabel('Conformal time (log scale)', fontsize=12)
    ...
    ax_v.scatter([t0], [vr0], color='cyan', s=80, zorder=5, marker='o')
    ax_v.scatter([t0], [vm0], color='orange', s=80, zorder=5, marker='o')
...
plt.suptitle('Perturbation Evolution (Logarithmic Time Scale)\nShowing Smooth Evolution from Big Bang to Recombination',
             fontsize=16, fontweight='bold')
plt.tight_layout()
plt.savefig(folder + 'perturbation_evolution_logscale.pdf', dpi=300, bbox_inches='tight')
print(f"\nPlot saved to {folder}perturbation_evolution_logscale.pdf")
plt.show()

print(f"\n{'='*70}")
print(f"PLOT COMPLETE - No jump visible on log scale!")
print(f"{'='*70}")
