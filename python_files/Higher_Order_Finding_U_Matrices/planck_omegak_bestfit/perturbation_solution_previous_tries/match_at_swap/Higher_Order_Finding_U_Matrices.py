...
#set tolerances
atol = 1e-13;  # tolerance for backward integration
rtol = 1e-13;  # tolerance for backward integration
atol_forward = 1e-10;  # tolerance for forward integration (less strict)
rtol_forward = 1e-10;  # tolerance for forward integration (less strict)
stol = 1e-10;
...
def process_single_k(k, s_init, endtime, swaptime, recConformalTime, deltaeta,
                     ...
                     num_variables, atol, rtol, atol_forward, rtol_forward, H0, z_rec):
    """
    Process a single k value to compute ABC, DEF, GHI matrices (backward)
    and JKL, MNO, PQR matrices (forward).
    ...
    atol, rtol : float
        Integration tolerances for backward integration
    atol_forward, rtol_forward : float
        Integration tolerances for forward integration
    H0 : float
        Hubble constant
    z_rec : float
    ...
    -------
    tuple
        (ABC_matrix, DEF_matrix, GHI_vector, JKL_matrix, MNO_matrix, PQR_matrix, X1, X2)
    """
    ...
    #---------------------------------------------------------------------------------------
    # For each K, find ABCDEFGHI matrices by integrating BACKWARD from endtime to swaptime
    #---------------------------------------------------------------------------------------
    ...
    for n in range(6):
        ...
        # Integrate backward from endtime to swaptime only
    ...
    #---------------------------------------------------------------------------
    # Find DEF matrix by integrating BACKWARD from endtime to swaptime
    #---------------------------------------------------------------------------
    ...
    #----------------------------------------------------------------------------
    # Find GHI vector by integrating BACKWARD from endtime to swaptime
    #----------------------------------------------------------------------------
    ...
    vec = np.array(sol5.y[:, -1])
    vec = np.delete(vec, 0)
    GHI_vector = vec

    #---------------------------------------------------------------------------------------
    # For each K, find JKLMNOPQR matrices by integrating FORWARD from recConformalTime to swaptime
    #---------------------------------------------------------------------------------------

    # Find initial conditions at recombination for forward integration
    a_rec = 1./(1+z_rec)

    JKL_matrix = np.zeros(shape=(num_variables, 6))
    ...
    for n in range(6):
        ...
        # Integrate forward from recConformalTime to swaptime (using BDF for stiff equations)
        solperf_forward = solve_ivp(dX3_dt_local, [recConformalTime, swaptime], inits_rec, method='BDF', atol=atol_forward, rtol=rtol_forward, max_step=1.e-2)

        nth_col = []
        for m in range(1, num_variables+1):
            nth_col.append(solperf_forward.y[m, -1])

        JKL_matrix[:, n] = nth_col

    #---------------------------------------------------------------------------
    # Find MNO matrix by integrating FORWARD from recConformalTime to swaptime
    #---------------------------------------------------------------------------

    MNO_matrix = np.zeros(shape=(num_variables, 2))
    ...
    for j in range(0, 2):
        ...
        # Integrate forward from recConformalTime to swaptime (using BDF for stiff equations)
        sol_forward = solve_ivp(dX3_dt_local, [recConformalTime, swaptime], inits_rec, method='BDF', atol=atol_forward, rtol=rtol_forward, max_step=1.e-2)

        nthcol = sol_forward.y[:, -1]
        nthcol = np.array(nthcol)
        nthcol = np.delete(nthcol, 0)
    ...
    #----------------------------------------------------------------------------
    # Find PQR matrix by integrating FORWARD from recConformalTime to swaptime
    # Loop over higher-order multipoles Fr4, Fr5, ..., Fr72
    #----------------------------------------------------------------------------

    PQR_matrix = np.zeros(shape=(num_variables, num_variables - 8))

    for j in range(0, num_variables - 8):
        x0 = np.zeros(num_variables)
        inits_rec = np.concatenate(([a_rec], x0))
        inits_rec[j + 9] = 1  # Set Fr4, Fr5, ..., Fr72 to 1 at recombination
    ...
    return ABC_matrix, DEF_matrix, GHI_vector, JKL_matrix, MNO_matrix, PQR_matrix, X1, X2
...
# Unpack results
for ABC_matrix, DEF_matrix, GHI_vector, JKL_matrix, MNO_matrix, PQR_matrix, X1, X2 in results:
    ABCmatrices.append(ABC_matrix)
    DEFmatrices.append(DEF_matrix)
    GHIvectors.append(GHI_vector)
    JKLmatrices.append(JKL_matrix)
    MNOmatrices.append(MNO_matrix)
    PQRmatrices.append(PQR_matrix)
...
np.save(folder_path+'L70_JKLmatrices', JKLmatrices);
np.save(folder_path+'L70_MNOmatrices', MNOmatrices);
np.save(folder_path+'L70_PQRmatrices', PQRmatrices);
