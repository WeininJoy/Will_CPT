...
# Perfect fluid ODE for early times (using same approach as Higher_Order_Finding_Xrecs.py)
def dX1_dt(t, X, k):
    """
    ODE system for perturbation variables from Big Bang to recombination.
    X = [a, phi, dr, dm, vr, vm]
    """
...
for i in range(num_modes_to_plot):
    ...
    print("Recombination (backward):", Y_backward[:, ::-1][:, -1]) # Last column corresponds to recTime
    
    # --- e) Get solution from Big Bang to Recombination using root-finding approach ---
    # (Following the method in Higher_Order_Finding_Xrecs.py)
    #------------------------------------------------------------------------------
    # Set up initial conditions for other variables using Taylor expansions
    #------------------------------------------------------------------------------
    ...
    try:
        ...
        print(f"  Root-finding converged! Optimal phi0 = {phi0_optimal}")
    except ValueError:
    ...
    # Extract values at recombination for verification
    ...
    print(f"  Verification: phi_rec = {phi_rec_final:.6e}, phi_constraint = {phi_constraint_final:.6e}")
    print(f"  Difference = {abs(phi_rec_final - phi_constraint_final):.6e}")
    print("  Recombination (perfect):", sol_perfect.y[:, -1])
...
plt.savefig(folder_path + 'perturbation_solutions_transMatrix_timeseries_phiTaylor.pdf')
plt.show()