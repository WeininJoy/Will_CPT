...
for j in range(len(kvalues)):
    ...
    AX1 = A.reshape(6,6) @ X1.reshape(6,5);
    DX2 = D.reshape(6,2) @ X2.reshape(2,5);
    matrixog = AX1 + DX2 # + GX3;
    matrix = matrixog[[0,2,3,4,5], :]; # keep phi, dr, dm, vr, vm rows only
    
    xrecs = [recs[1], recs[2], recs[3], recs[4], recs[5]];

    xinf = np.linalg.lstsq(matrix, xrecs, rcond=None)[0];
    phidotfcb.append(xinf[0]);
    vrfcb.append(xinf[3]);
    drfcb.append(xinf[1]);
    dmfcb.append(xinf[2]);
    vmdotfcb.append(xinf[4]);
