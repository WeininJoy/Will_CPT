...
rtol = 1e-13
stol = 1e-10
num_variables = 75 - 2; # number of pert variables (excluding phi and psi)
...
def dX2_dt(t, X):
    ...
    for j in range(6, num_variables):  # Start from index 6 (F_3) in reduced array
        l = j - 3  # l = 3 when j = 6
...
for i in range(len(kvalues)):
    ...
    for n in range(4):
        ...
        solperf = solve_ivp(dX2_dt, [endtime,swaptime], inits, dense_output=True, method='RK45', atol=atol, rtol=rtol);
        #print(solperf.y[:,-1])
        ...
        solperf2 = solve_ivp(dX3_dt, [swaptime,recConformalTime], inits2, dense_output=True, method='RK45', atol=atol, rtol=rtol);
    ...
    for j in range(0,2): 
        ...
        sol3 = solve_ivp(dX2_dt, [endtime,swaptime], inits, method='RK45', atol=atol, rtol=rtol);
        ...
        sol4 = solve_ivp(dX3_dt, [swaptime,recConformalTime], inits2, method='RK45', atol=atol, rtol=rtol);
    ...
    sol5 = solve_ivp(dX2_dt, [endtime,swaptime], inits, method='RK45', atol=atol, rtol=rtol);
    
    inits2 = sol5.y[:,-1];
    inits2[0] = np.log(inits2[0]);
    
    sol6 = solve_ivp(dX3_dt, [swaptime, recConformalTime], inits2, method='RK45', atol=atol, rtol=rtol);
