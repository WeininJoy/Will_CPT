...
folder_path_matrices = './data_allowedK/' # For X1, X2, recValues
folder_path_timeseries = './data_allowedK_timeseries/' # For solution histories

# Cosmological parameters (unchanged)
OmegaLambda = 0.679
OmegaM = 0.321
OmegaR = 9.24e-5
...
atol = 1e-13
rtol = 1e-13
num_variables = 75 # This must match the num_variables in the pre-computation script
endtime = 6.15
deltaeta = 6.150659839680297 - endtime
Hinf = H0*np.sqrt(OmegaLambda)

# Background evolution (unchanged, needed for FCB time and perfect fluid part)
def ds_dt(t, s):
...
sol_bg = solve_ivp(ds_dt, [t0,12], [s0], max_step = 0.25e-4, method='LSODA', atol=atol, rtol=rtol)
print('Initial Background Integration Done')

idxfcb = np.where(np.diff(np.sign(sol_bg.y[0])) != 0)[0]
fcb_time = 0.5*(sol_bg.t[idxfcb[0]] + sol_bg.t[idxfcb[0] + 1])
s_rec = 1+z_rec
...
for i in range(num_modes_to_plot):
    ...
    s_background = np.interp(t_grid, sol_bg.t, sol_bg.y[0])
...
plt.savefig('perturbation_solutions_transMatrix_timeseries.pdf')
plt.show()
