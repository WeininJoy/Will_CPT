...
int primordial_init(
  ...
  if (ppm->primordial_spec_type == analytic_Pk) {
    ...
    /* Store background parameters for weighting calculations */
    ppm->K = pba->K;
    ppm->nu_spacing = pba->nu_spacing;
    ppm->sgnK = pba->sgnK;
...
int primordial_get_lnk_list(
  ...
  if  (ppm->sgnK <= 0) {
  ...
  } else {  // ppm->sgnK >0 (closed universe)
      ...
      while (index_k < k_size_max) {
          q = nu*sqrt(ppm->K);
          nu = nu + ppm->nu_spacing;
          k = sqrt(q*q - ppm->K*(m+1.));
          ppm->lnk[index_k] = log(k);
          fprintf(stdout, "%f ", ppm->lnk[index_k]);
          index_k++;
      }
...
/**
 * This routine reads k-dependent weights from file
 *
 * @param ppm            Input/Output: pointer to primordial structure
 * @return the error status
 */

int primordial_read_weights(
                           struct primordial * ppm
                           ) {
