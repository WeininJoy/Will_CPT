...
class Universe:
    ...
    def a(self, eta):
        if self.k_curved**2 < 1: # m is complex, choose m has positive impaginary part
    ...
    def plot_a(self):
        ...
        eta_list = np.linspace(-eta_tot, eta_tot, 100)
        plt.plot(eta_list, [self.a(eta).real for eta in eta_list], label = 'kc='+str(round(self.k_curved, 2)))
    ...
    def plot_Phi(self, Phi_i, Phi_dot_i, k_scale):
        ...
        def plot_Phi_anal():
            ...
            plt.plot(a_list, Phi_list, label="anal, kc="+str(round(self.k_curved, 2)))
...
for k_curved in k_curved_list:
    universe = Universe(Lambda=1., R=1., k_curved=k_curved)
