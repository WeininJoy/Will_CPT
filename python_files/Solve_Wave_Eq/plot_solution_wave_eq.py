...
if __name__=="__main__":
    ...
    eigenvalues_valid_1 = np.loadtxt(f"./data/diff_m/eigenvalues1_2d_N{N:d}_Nt{N_t:d}_T{T:.2f}_m{kappa:.2f}.txt", dtype=np.complex128)
    coefficients_1 = np.loadtxt(f"./data/diff_m/coefficients1_2d_N{N:d}_Nt{N_t:d}_T{T:.2f}_m{kappa:.2f}.txt", dtype=np.complex128)
    ...
    eigenvalues_valid_2 = np.loadtxt(f"./data/diff_m/eigenvalues2_2d_N{N:d}_Nt{N_t:d}_T{T:.2f}_m{kappa:.2f}.txt", dtype=np.complex128)
    coefficients_2 = np.loadtxt(f"./data/diff_m/coefficients2_2d_N{N:d}_Nt{N_t:d}_T{T:.2f}_m{kappa:.2f}.txt", dtype=np.complex128)
    ...
    # plt.legend()
    # plt.xlabel(r"$t$")
    # plt.savefig(f"figures/eigenvalue_2d_N{N:d}_Nt{N_t:d}_T{T:.2f}_m{kappa:.2f}_xpi_sorted_by_eigenvalue.pdf")
    ...
    # plt.legend()
    # plt.xlabel(r"$x$")
    # plt.savefig(f"figures/eigenvalue_2d_N{N:d}_Nt{N_t:d}_T{T:.2f}_m{kappa:.2f}_t0.5pi.pdf")


    # ## 3D plot of the solution Phi(t,x) 
    # # Contour plot (sequence)
    # fig, axs = plt.subplots(N_plot//3, 3, figsize=(13.5,6))
    # cmap = plt.get_cmap('RdGy')
    # for i in range(N_plot//3):
    #     for j in range(3):
    ...
    # fig.tight_layout()
    # plt.savefig(f"/figures/eigenvalue_2d_N{N:d}_Nt{N_t:d}_T{T:.2f}_m{kappa:.2f}_contour_sequences.pdf")
    ...
    plt.savefig(f"figures/eigenvalue_2d_N{N:d}_Nt{N_t:d}_T{T:.2f}_m{kappa:.2f}_contour1.pdf")
    ...
    fig.tight_layout()
    plt.savefig(f"figures/eigenvalue_2d_N{N:d}_Nt{N_t:d}_T{T:.2f}_m{kappa:.2f}_cylinder1.pdf")
