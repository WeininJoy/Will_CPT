...
int background_at_z(
  ...
  /** - check that log(a) = log(1/(1+z)) = -log(1+z) is in the pre-computed range (allowing boundary values) */
  loga = -log(1+z);
...
int background_tau_of_z(
  ...
  /** - check that \f$ z \f$ is in the pre-computed range (allowing boundary values) */
  class_test(z < pba->z_table[pba->bt_size-1] && fabs(z - pba->z_table[pba->bt_size-1]) > 1e-10,
             pba->error_message,
             "out of range: z=%e < z_min=%e\n",z,pba->z_table[pba->bt_size-1]);

  class_test(z > pba->z_table[0] && fabs(z - pba->z_table[0]) > 1e-10,
             pba->error_message,
...
int background_z_of_tau(
  ...
  /** - check that \f$ tau \f$ is in the pre-computed range (allowing boundary values) */
  class_test(tau < pba->tau_table[0] && fabs(tau - pba->tau_table[0]) > 1e-10,
             pba->error_message,
             "out of range: tau=%e < tau_min=%e\n",tau,pba->tau_table[0]);

  class_test(tau > pba->tau_table[pba->bt_size-1] && fabs(tau - pba->tau_table[pba->bt_size-1]) > 1e-10,
             pba->error_message,
...
int background_solve(
  ...
  loga_final = log(pba->a_final_over_a_today); // with our conventions, loga is in fact log(a/a_0); we integrate until a_final_over_a_today
