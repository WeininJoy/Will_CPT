...
class Universe:
    def __init__(self, Lambda, R, k_curved, n): # k_curved: close(>0), open(<0), flat(0) , abs(value) < 1 is physical 
    ...
    def s(self, eta):
        return 1. / self.a_eta(eta).real

    def plot_a(self):
        ##### make plot of the real part of analytic solution
        eta_tot = self.eta_tot()
        a_list = [self.a(1.j*eta) for eta in np.linspace(1.e-5, 2*eta_tot*(1.-1.e-5), 100)]
        print(a_list)
        # plt = cplot.plot(self.a, (-1.5*eta_tot, 1.5*eta_tot, 400), (-1.5*eta_tot, 1.5*eta_tot, 400))
        # plt.savefig(f"a-eta_kc={self.k_curved:.1f}.pdf")
...
universe = Universe(Lambda=1., R=1., k_curved=0.5, n=0)
universe.plot_a()
