...
import pickle
import time
from calculate_allowedK.Higher_Order_Finding_U_Matrices import findind_U_matrices
from calculate_allowedK.Higher_Order_Finding_Xrecs import findind_Xrecs
from calculate_allowedK.Higher_Order_Solving_for_Vrinf import solving_Vrinf
import classy
...
# Parameters
params_file = '/home/wnd22/rds/hpc-work/Will_CPT/CMB_CLASS/data/Planck_params/continuous_params_omegak.txt'
allowed_k_file = '/home/wnd22/rds/hpc-work/Will_CPT/CMB_CLASS/class_quantised/allowed_k.txt'
...
def find_max_tried_num():
    ...
    directory = "/home/wnd22/rds/hpc-work/Will_CPT/CMB_CLASS/data/best-fit_params_zrec/"
...
import os.path
bestfit_file_root = '/home/wnd22/rds/hpc-work/Will_CPT/CMB_CLASS/data/best-fit_params_zrec/bestfit_params.txt'
...
def run_class(find_params):
    ...
    Lambda = Omega_lambda * 3 * H0**2 / c**2  # unit: km2 / (Mpc2 m2)
    sqrtlambda = np.sqrt(Lambda.value * 1.e6)
    z_rec = calculate_z_rec(find_params)

    ################
    # get Delta K values by interpolation (based on the data)
    DeltaK = interpolate_Deltak([mt, kt, z_rec])
...
def check_prior(find_params):
    ...
    prior = np.array([[300, 450], [1.e-4, 3], [0.15, 0.17], [0.5, 0.75], [2.0, 2.2], [0.95, 0.98], [0.0386, 0.065]]) # mt, kt, omega_b_ratio, h, A_s, n_s, tau
...
def run_TT(find_params, bestfit_file_root):
    ...
    try:
        ...
        if np.isnan([val for val in cls.values()]).any(): # if there is any Nan in cls -> raise CosmoComputationError
            ...
            global try_num
            filename = f'/home/wnd22/rds/hpc-work/Will_CPT/CMB_CLASS/data/best-fit_params_zrec/try_params_{try_num}.txt'
            with open(filename, 'w') as f:
                print(plik, lowl, lowE, chi_eff_sq, *find_params, 'False', file=f)
            try_num += 1
...
def get_data(bestfit_file_root):
    ...
    with open(bestfit_file_root, 'w') as f:
        print(plik, lowl, lowE, chi_eff_sq, *res.x, res.success, file=f)
