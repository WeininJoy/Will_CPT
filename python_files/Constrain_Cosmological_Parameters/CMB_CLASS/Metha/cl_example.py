...
kt = 0.85 # dimensionless curvature = kappa/lam
...
"""
# import discrete k values from files
"""
...
delta_kint = 3;
deltak = 3 * np.sqrt(kt) * sqrtlambda;

nmax = int(kmax/deltak);
k = [i * deltak for i in range(nmax)]
...
with open(quant_cl_file, 'w') as f:
...
fig_width_pt = 218.1*1.8;  # Get this from LaTeX using \showthe\columnwidth
...
label = [r'$\Lambda$CDM',\
        'Quantised']
ax1a.legend(custom_lines, label, loc=9, bbox_to_anchor=(1, 2), fancybox=True, ncol=2)
