...
# kt_list = np.linspace(0.4, 0.75, 20)
...
def run_class(params, input_number):
    ...
    s0, Omega_lambda, Omega_m, Omega_K = cosmological_parameters(input_number)
    h = params[3]
...
for i in [2,12]:
    for j in range(20):
        input_number = 100*i + (j+20)
        mt = mt_list[i]
        kt = kt_list[j]
        k0, delta_k, plik, lowl, lowE, lensing, chi_eff_sq, success = run_TT(params, input_number)
        result = [mt, kt, k0, delta_k, plik, lowl, lowE, lensing, chi_eff_sq] 
        result = np.concatenate((result, params))
        result = np.concatenate((result, [success]))
        file = open(f'/home/wnd22/rds/hpc-work/Will_CPT/Constrain_Cosmological_Parameters/CMB_CLASS/Metha/data/cl_files/fix_OmegaR/cl_{input_number}.txt','w')
        for param in result:
            file.write(param+" ")
        file.close()