...
kt_list = np.linspace(0.75, 1.1, 20)
...
def cosmological_parameters(h, input_number): # h = H0/100
    mt = mt_list[input_number//100]
    kt = kt_list[input_number%100-20]
...
# Runs the CLASS code
def run_class(find_params, input_number):

    h = find_params[0]
    s0, Omega_lambda, Omega_m, Omega_K = cosmological_parameters(h, input_number)
    H0 = h*100 * u.km/u.s/u.Mpc
    Lambda = Omega_lambda * 3 * H0**2 / c**2  # unit: km2 / (Mpc2 m2)
    sqrtlambda = np.sqrt(Lambda.value * 1.e6)
    allowedK = np.load(f'/home/wnd22/rds/hpc-work/Will_CPT/Higher_Order_Finding_U_Matrices/Curve_universes_a0/data/fixOmegaR/allowedK_{input_number}.npy')
    ...
    LambdaCDM.set({'omega_b':params[0]/(params[1]+params[0])*Omega_m*h**2, # omega_b = omega_b/(omega_cdm + omega_b) *omega_m
...
def check_prior(find_params):

    prior = np.array([[0.63, 0.74]])  #H0

    diff_below = find_params - prior[:, 0]
    diff_above = prior[:, 1] - find_params
...
def run_TT(find_params, input_number, filename):
        
    if check_prior(find_params) == True:

        try:
            # Find corresponding spectra
            cls = run_class(find_params, input_number)
            ...
            if np.isnan([val for val in cls.values()]).any(): # if there is any Nan in cls -> raise CosmoComputationError
                ...
                print('h, chi_eff_sq= %s, %s' % (str(find_params[0]), str(chi_eff_sq)))
    ...
    if chi_eff_sq < best_chisq:
        ...
        with open(filename, 'w') as f:
            print(plik, lowl, lowE, lensing, chi_eff_sq, *find_params, 'False', file=f)

    return plik, lowl, lowE, lensing, chi_eff_sq

def get_data(input_number, bestfit_file_root):

    def to_optimise(find_params):
        plik, lowl, lowE, lensing, chi_eff_sq = run_TT(find_params, input_number, bestfit_file_root)
        return chi_eff_sq

    res = minimize(to_optimise, start_params, method='Nelder-Mead', options={'fatol': 0.03, 'xatol':0.001})
    plik, lowl, lowE, lensing, chi_eff_sq = run_TT(res.x, input_number, bestfit_file_root)
...
if __name__ == '__main__':
    ...
    for num in range(20):
        input_number = int(sys_input*100 + num+20)
        print("input_number = ", input_number)
        bestfit_file_root = f'/home/wnd22/rds/hpc-work/Will_CPT/Constrain_Cosmological_Parameters/CMB_CLASS/Metha/data/best-fit_H0/bestfitH0_{input_number}.txt'
        get_data(input_number, bestfit_file_root)